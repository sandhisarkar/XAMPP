...
$q=mysqli_query($con,"select username,state from tbl_operator where operator_id ='".$_SESSION['user']."'");
...
if(($hour >= 17 && $min <= 59) && ($hour < 21 && $min <= 59))
{
    $enablestat = true;
    $ggg = "ok";
}
else if(($hour >= 17 && $min <= 59) && ( ($hour <= 21 && $min <= 59) || ($hour == 22 && $min <=29) ) && $state == "Assam")
{
    $enablestat = true;
    $ggg = "ok";
}
...
{
    $operator_id = $_SESSION['user'];
	  $updatecorrect=$_POST['updatecorrect'];
    $enrollment=$_POST['enrollment'];
    $dateapplication= $year."-".$month."-".$day;
    ...
    if($state != "Assam")
    {
      if(($hour >= 17 && $min <= 59) && ($hour < 21 && $min <= 59))
      {
        ...
        //todays date comparison
          if($year == substr($datetime, 0,4) && $month == substr($datetime,5,2) && $day == substr($datetime,8,2))
          { 
            $created_dttm = $year."-".$month."-".$day;
            $result=mysqli_query($con ,"select * from tbl_attendance where operator_id = '".$operator_id."' and created_dttm = '".$created_dttm."'");
            ...
            {
              $endiff = 0;
              $query_day_count = mysqli_query($con ,"SELECT COUNT(slno) FROM `tbl_attendance` WHERE created_dttm = '".$created_dttm."'");
              ...
              $dt1 = strtotime($created_dttm);

              $dt2 = date('D', $dt1);

              if($dt2 == "Sun")
              {
                $result=mysqli_query($con ,"select * from tbl_outrangetime where operator_id = '".$operator_id."' and created_date = '".$created_dttm."'");
                    
                $formatedDTTM = "00".":"."00".":"."00"." Hrs";
                $item_no = 1;
                $insertOutRange_sql="INSERT INTO tbl_outrangetime (item_no,operator_id,created_date,log_dttm) VALUES ('".$item_no."','".$operator_id."','".$created_dttm."','".$formatedDTTM."')";
              ...
              $insert_sql="INSERT INTO tbl_attendance (slno,operator_id,updatecorrection,enrollment,original_enrollment,csv_upco,csv_newen,en_diff,dept,state,district,pec_location,stid_db,stid_csv,created_dttm,remarks) VALUES ('".$slno."','".$operator_id."','".$updatecorrect."','".$enrollment."','0','0','0','".$endiff."','".$db_dept."','".$db_state."','".$db_district."','".$db_pec_location."','".$db_stid."','".$csv_stid."','".$created_dttm."','".$remark."')";
    ...
    {
      if(($hour >= 17 && $min <= 59) && (($hour <= 21 && $min <= 59) || ($hour == 22 && $min <= 29)))
      {
          $enablestat = true;
          $ggg = "ok";
        //todays date comparison
  ...
  <body>
            ...
            <div class="col-lg-4">
              ...
              <div class="card">
                <div class="card-header d-flex align-items-center">
                  ...
                  {
                    ?>
                    <h4>Reporting (Time : 5 PM to 10:30 PM)</h4>
                    <?php
                  }
                  else
                  {
                    ?>
                    <h4>Reporting (Time : 5 PM to 9 PM)</h4>
                    <?php
                  }
                  ?>
                  ...
                  <form style="border-style: none; display: none;" action = "" method ="post"  id = "ReportingCSV" enctype="multipart/form-data">
                    <?php
                        ...
                        else
                        {
                            ...
                            <div class="form-group">
                            <label>Update / Correction </label>
                            <input type="number" disabled required ="true" min="0" max=""
                            placeholder="Enter any update or correction" id="updatecorrect" name = "updatecorrect" class="form-control">
                            </div> 
                            <div class="form-group">
                            <label>New Enrollment </label>
                            ...
                            <div class="form-group">
                              <label>Choose Reporting CSV </label>
                              <input type="file" disabled required ="true"
                              accept=".csv"
                              placeholder="Excel Upload" id="excel_file" name = "excel_file" class="form-control">
                            </div>
                            <div class="form-group">
                              <label>Remarks </label>
                              <textarea class="form-control"
                              placeholder="Any remarks" id="remarks" disabled name = "remarks" ></textarea>
                            </div>  
                            <div class="form-group">       
                            <input type="submit" value="Submit Reporting" style="float: right;" disabled class="btn btn-primary" id="sub_app" name = "sub_app" onclick = "uploadme()">
                            </div>
                            <div class="form-group" style="text-align: center;">    
                                <label class="text-danger" >You cannot apply for the Reporting...</label>
                            </div>        
                  ...
                  <form style="border-style: none; display: none;" action = "" method ="post"  id = "Reporting" enctype="multipart/form-data">
                    <?php
                        ...
                        else
                        {
                            ...
                            <div class="form-group">
                            <label>Update / Correction </label>
                            <input type="number" disabled required ="true" min="0" max="0"
                            placeholder="Enter any update or correction" id="updatecorrect" name = "updatecorrect" class="form-control">
                            </div> 
                            <div class="form-group">
                            <label>New Enrollment </label>
                            ...
                            <div class="form-group">
                              <label>Remarks </label>
                              <select multiple="" class="form-control" id="remarks" disabled name = "remarks[]" required ="true">
                                  <option style="color:black;">Absent / On leave</option>
                                  <option style="color:black;">Electricity problem</option>
                                  <option style="color:black;">Device problem</option>
                                  <option style="color:black;">ID Inactive</option>
                            ...
                            <div class="form-group">       
                              <input type="submit" value="Submit Reporting" style="float: right;" disabled class="btn btn-primary" id="sub_app_zero" name = "sub_app_zero" onclick = "uploadme()">
                            </div>
                            <div class="form-group" style="text-align: center;">    
                                <label class="text-danger" >You cannot apply for the Reporting...</label>
                            </div>        
            ...
            if($error == "Query")
						{
							//header( "refresh:3;url=Operator.php" );
							echo '<div class="spinner-grow spinner-grow-sm"> </div><span> &nbsp;&nbsp;&nbsp;</span>';
							echo '<label class="text-danger">Reporting not submitted '.$insert_sql.'</label>';
            ...
            if($error == "Not For Date")
						{
							//header( "refresh:3;url=Operator.php" );
							echo '<div class="spinner-grow spinner-grow-sm"> </div><span> &nbsp;&nbsp;&nbsp;</span>';
							echo '<label class="text-danger">Reporting is not submitted .</br>Check uploadable CSV File again along with current date and operator_id in CSV </label>';
						...
						if($error == "Exists")
						{
							...
							echo '<div class="spinner-grow spinner-grow-sm"> </div><span> &nbsp;&nbsp;&nbsp;</span>';
							echo '<label class="text-danger">Reporting is already been registered for today</label>';
							//echo $output;
						...
						if($success == "Success")
						{
							//header( "refresh:3;url=Operator.php" );
							echo '<div class="spinner-grow spinner-grow-sm"> </div><span> &nbsp;&nbsp;&nbsp;</span>';
							echo '<label class="text-success">Reporting is successfully registered for today</label>';
							//header( "refresh:3;url=Operator.php" );
						}
					?>
				</div>
    ...
    <script language ="javascript">
		function uploadme()
		{
          ...
          if(updatecorrect === "" || enrollment === "" || csv ==="")
          {
            
            document.getElementById("overlay").style.display = "none";
            return false;
          }
          if(csv!==null && updatecorrect !== null && enrollment !== null )
          {

...
if(isset($_POST["sub_app_search"]))
{
    ...
    if($district == "All")
    {
      ...
      $result=mysqli_query($con ,"SELECT distinct(a.operator_id) from tbl_attendance a, tbl_operator b where a.operator_id = b.operator_id and a.state = '".$state."' and a.operator_id = '".$operator."' and a.created_dttm BETWEEN '".$datestartapplication."' and '".$dateendapplication."' ");
    ...
    {
      $created_dttm = $year."-".$month."-".$day;
      $result=mysqli_query($con ,"SELECT distinct(a.operator_id) from tbl_attendance a, tbl_operator b where a.operator_id = b.operator_id and a.state = '".$state."' and a.district = '".$district."' and a.operator_id = '".$operator."' and a.created_dttm BETWEEN '".$datestartapplication."' and '".$dateendapplication."' ");
  ...
  <body>
          ...
          <ul id="side-main-menu" class="side-menu list-unstyled">                  
            ...
            <li class="active"><a href="#exampledropdownDropdown1" aria-expanded="false" data-toggle="collapse"> <i class="fa fa-file-excel-o"></i>Report </a>
              <ul id="exampledropdownDropdown1" class="collapse list-unstyled ">
                ...
                <li class="active"><a href="Supervisor.php" onclick="openOverlay()"><i class="fa fa-user-o"></i>Supervisor</a></li>
    ...
    <div class="page">
      ...
      <section class="forms">
        <div class="container-fluid">
          <!-- Page Header-->
          <header> 
            <h1 class="h3 display">Supervisor wise report. . .</h1>
            <p>(Search result for specific supervisor)</p>
          </header>
          <div class="row">
            <div class="col-lg-4">
            </div>
            ...
            <div class="col-lg-4">
              <div class="card">
                ...
                <div class="card-body">
                  <p>(Details report for specific supervisor)</p>
                  <?php
                  if($role == 1)
                  {
                    ?>
                    ...
                    <?php
                        if($enablestat == true || $enablestat == false) 
                        {
                            ...
                            <label>Choose Supervisor </label>
                                <select enable ="<?php $enablestat; ?>" required ="true" placeholder="Choose District" id="operator" name="operator" autofocus class="form-control">
                                    <option disabled selected value = "">Select Any...</option>
                                </select>
                            </div>
            ...
            <div class="col-sm-12" style = "text-align: center; margin-left: auto;margin-right: auto;">
					<?php
						...
						if($success == "Exists")
						{
                            ...
                            //echo $output;
                            ?>
								<form style="border-style: none;" action = "genSupervisor.php" method ="post"  id = "export_excel" enctype="multipart/form-data">
                                ...
                                <div id = "range_reporting_table">
                                    ...
                                    <?php
                                      ...
                                      {
                                        $sql = "SELECT a.state,a.district,a.dept,a.operator_id,b.username,a.pec_location,a.stid_db,a.stid_csv,a.updatecorrection,a.csv_upco,a.enrollment,a.csv_newen,a.original_enrollment,a.en_diff,a.remarks,date_format(a.created_dttm,'%d-%M-%Y')  from tbl_attendance a, tbl_operator b where a.operator_id = b.operator_id and a.state = '".$state."' and a.district = '".$district."' and a.operator_id = '".$operator."' and a.created_dttm BETWEEN '".$datestartapplication."' and '".$dateendapplication."' order by a.created_dttm";
                                        $result = mysqli_query($con, $sql);
                                        while($row = mysqli_fetch_row($result)) {
                                        ?>
                                         <tbody id="myTable">
						...
						if($error == "Not Exists")
						{
							//header( "refresh:3;url=Operator.php" );
							echo '<div class="spinner-grow spinner-grow-sm"> </div><span> &nbsp;&nbsp;&nbsp;</span>';
							echo '<label class="text-danger">No Reporting result found for Supervisor - '.$operator.'</label>';
							//header( "refresh:3;url=Operator.php" );
						}
					?>
				</div>
    ...
    <script language ="javascript">
        function uploadme() {
            ...
            if(state === "" || operator === "" || repstdate === "" || rependdate === "")
            {
                
                document.getElementById("overlay").style.display = "none";
                return false;
            }
            if(state !== null && operator !== null && repstdate !== null && rependdate !== null)
            {
                document.getElementById("overlay").style.display = "block";
                return true;
            }
      ...
      <script>
        function uploadEditmeNormal(st,operator)
		{
          var state = st;
          ...
          if(state !== null && operator !== "" )
          {
            
            document.getElementById("overlay").style.display = "none";
            var excel_data = $('#range_reporting_table').html();
            var page = "genSupervisor.php?data="+excel_data;

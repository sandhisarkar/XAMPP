...
if(isset($_POST["sub_excel"]))
{
	...
	if(!empty($_FILES["pf_document"])  && !empty($_FILES["esi_document"]))
	{
		if(($_FILES["pf_document"]["type"] == ".pdf" || $_FILES["pf_document"]["type"] == "application/pdf") && ($_FILES["esi_document"]["type"] == ".pdf" || $_FILES["esi_document"]["type"] == "application/pdf"))
		{
			chdir("..");
			
			
			$pfdocpath = "Admin/Documents/Challan/".$month_yr."/PF/";
			
			
			if(!is_dir($pfdocpath))
			{
				//echo 1;
				mkdir($pfdocpath,"0777",true);
			...
			$esidocpath = "Admin/Documents/Challan/".$month_yr."/ESI/";
			
			
			if(!is_dir($esidocpath))
			{
				//echo 1;
				mkdir($esidocpath,"0777",true);
			...
			$pfdocpath=$pfdocpath.$_FILES['pf_document']['name'];
			$esidocpath=$esidocpath.$_FILES['esi_document']['name'];
			
			if(move_uploaded_file($_FILES['pf_document']['tmp_name'],$pfdocpath) && move_uploaded_file($_FILES['esi_document']['tmp_name'],$esidocpath))
			{
				$pfdoc = $_FILES['pf_document']['name'];
				$esidoc = $_FILES['esi_document']['name'];
				
				$sql="UPDATE salary_month_details SET pf_esi_challan_status = 'Completed', pf_doc = '".$pfdocpath."', esi_doc = '".$esidocpath."' WHERE month_year='".$month_yr."'";
...
<html lang="en">
  ...
  <script>
      ...
      <div id="content" class="p-4 p-md-5 pt-5" >
        ...
        <h2 class="mb-4"><u>PF/ESI Challan Upload</u> (Monthly)</h2>
			...
			<div class="col-sm-12" style = "margin-top:5%;">
				...
				<div class="col-sm-6" style = "text-align: center; margin-left: auto;margin-right: auto;">
					...
					<center>
						...
						<form  action = "PF_ESI Upload.php" method ="post"  id = "export_excel" enctype="multipart/form-data">
							<table>
								...
								<td style = "text-align: center;margin-left: auto;margin-right: auto;">
									...
									<select required="true" id="month_year" name="month_year" style="font-size: 15px;height: 25px;padding-top: 0px;margin-top: 25px;width: 300px;">
										...
										<?php
											...
											$sql = "SELECT DISTINCT(month_year) FROM salary_month_details where pf_esi_challan_status  = 'Pending'";
									...
									<label style="margin-left: -5%; margin-bottom:5%;margin-top:5%; color: #495057;">Upload PF Document : &nbsp; </label>
									<input required ="true" accept = ".pdf,application/pdf" class="toggle btn btn-primary" type="File" id="pf_document" name = "pf_document" value= "" autofocus style="margin-bottom: 7px;"><!--span class="text-success">&nbsp;<?php echo $uidaiproof;?></span--></input></br>
									
									<label style="margin-left:-6%;color: #495057;">Upload ESI Document :  &nbsp; </label> 
														
									<input required ="true" accept = ".pdf,application/pdf" class="toggle btn btn-primary" type="File" id="esi_document" name = "esi_document" value= "" autofocus style="margin-bottom: 7px;"><!--span class="text-success">&nbsp;<?php echo $uidaiproof;?></span--></input></br>
									
									
									<button class="toggle btn btn-primary" onclick = "uploadme()" type="submit" id="sub_excel" name = "sub_excel"
									style="font-family: Verdana;  margin-top:15px; background-color: #1163aa; border: transparent; margin-bottom: 15px;">Upload <i class="fa fa-upload"></i></button>
				...
				<div class="row" style="margin-top: 5%;">
					<?php
						...
						if($msgNav == "Success")
						{
							...
							<div class="col-md-12  wow bounceInUp" data-wow-duration="2.4s">
							  <div class="box" >
								<h6><i class="fa fa-file text-success"></i> File Uploaded Successfully . . .  <i class="fa fa-upload text-success"></i></h6>
	...
	<script>
		...
		function uploadme()
		{
			var pfdoc = $("#pf_document").val();
			var esidoc = $("#esi_document").val();
			...
			if(allowedPdfExtensions.exec(pfdoc) == ".pdf,.pdf"  && allowedPdfExtensions.exec(esidoc) == ".pdf,.pdf" && date !=="")
			{

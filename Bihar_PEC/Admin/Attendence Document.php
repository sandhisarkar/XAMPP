...
if(isset($_POST["sub_excel"]))
{
	...
	$foldername = $district."_".$month_yr;
	
	
	
	if(!empty($_FILES["document"]))
	{
		if($_FILES["document"]["type"] == ".pdf" || $_FILES["document"]["type"] == "application/pdf")
		{
			chdir("..");
			
			
			$docpath = "Admin/Documents/Attendance/".$month_yr."/".$district."/";
			...
			$docpath=$docpath.$_FILES['document']['name'];
			
			if(move_uploaded_file($_FILES['document']['tmp_name'],$docpath))
			{
				$docproof = $_FILES['document']['name'];
				
				
				$sql="UPDATE monthly_attendence SET status= 'Pending', doc = '".$docpath."' WHERE month_year='".$month_yr."' AND district_name = '".$district."'";
...
<html lang="en">
  ...
  <script>
      ...
      <div id="content" class="p-4 p-md-5 pt-5" >
        ...
        <h2 class="mb-4"><u>Document Upload</u> (Monthly Attendance Sheet)</h2>
			...
			<div class="col-sm-12" style = "margin-top:5%;">
				...
				<div class="col-sm-6" style = "text-align: center; margin-left: auto;margin-right: auto;">
					...
					<center>
						...
						<form  action = "Attendence Document.php" method ="post"  id = "export_excel" enctype="multipart/form-data">
							<table>
							...
							<tr>
								<td style = "text-align: center;margin-left: auto;margin-right: auto;">
									...
									<select required="true" id="month_year" onChange = "getName(this.value);" name="month_year" style="font-size: 15px;height: 25px;padding-top: 0px;margin-top: 25px;width: 300px;">
										<option disabled selected value = "">Select Any...</option>
										<?php
											//mysqli_select_db( $con,"course_details");

											$sql = "SELECT DISTINCT(month_year) FROM monthly_attendence where status = 'Created'";
									...
									<label style="margin-left: 3%; margin-bottom:5%;color: #495057;">Choose District : &nbsp; </label>
									<select required="true" id="district_name" name="district_name" style="font-size: 15px;height: 25px;padding-top: 0px;margin-top: 25px;width: 300px;">
										
										<option disabled selected value = "">Select Any...</option>
										
									</select></br>
									
									<label style="margin-left:0%;color: #495057;">Upload Document :  &nbsp; </label> 
														
									<input required ="true" accept = ".pdf,application/pdf" class="toggle btn btn-primary" type="File" id="document" name = "document" value= "" autofocus style="margin-bottom: 7px;"><!--span class="text-success">&nbsp;<?php echo $uidaiproof;?></span--></input></br>
						...
						if($msgNav == "Success")
						{
							...
							<div class="col-md-12  wow bounceInUp" data-wow-duration="2.4s">
							  <div class="box" >
								...
								<h5><i class="fa fa-map text-warning"></i> District - <u><b><?php  echo $district;?></b></u></h5>
							  </div>
						  </div>
						  <?php
							echo $output;
	...
	<script>
		...
		function uploadme()
		{
			...
			if(allowedPdfExtensions.exec(doc) == ".pdf,.pdf" && date !=="" && district_name !== "")
			{
			...
			else if(doc !== "" && date !=="" && district_name !== "")
			{
				document.getElementById("overlay").style.display = "block";
				return true;

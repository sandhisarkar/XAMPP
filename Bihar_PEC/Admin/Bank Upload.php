...
if(isset($_POST["sub_excel"]))
{
	...
	if(!empty($_FILES["bank_document"]))
	{
		if(($_FILES["bank_document"]["type"] == ".pdf" || $_FILES["bank_document"]["type"] == "application/pdf"))
		{
			chdir("..");
			
			
			$bankdocpath = "Admin/Documents/Bank Advice/".$month_yr."/";
			
			
			if(!is_dir($bankdocpath))
			{
				//echo 1;
				mkdir($bankdocpath,"0777",true);
			...
			$bankdocpath=$bankdocpath.$_FILES['bank_document']['name'];
			
			
			if(move_uploaded_file($_FILES['bank_document']['tmp_name'],$bankdocpath))
			{
				$bankdoc = $_FILES['bank_document']['name'];
				
				
				$sql="UPDATE salary_month_details SET bank_advice_status = 'Completed', bank_advice_doc = '".$bankdocpath."' WHERE month_year='".$month_yr."'";
...
<html lang="en">
  ...
  <script>
      ...
      <div id="content" class="p-4 p-md-5 pt-5" >
        ...
        <h2 class="mb-4"><u>Upload Bank Payment Document</u> (Monthly)</h2>
			...
			<div class="col-sm-12" style = "margin-top:5%;">
				...
				<div class="col-sm-6" style = "text-align: center; margin-left: auto;margin-right: auto;">
					...
					<center>
						...
						<form  action = "Bank Upload.php" method ="post"  id = "export_excel" enctype="multipart/form-data">
							<table>
								...
								<td style = "text-align: center;margin-left: auto;margin-right: auto;">
									...
									<select required="true" id="month_year" name="month_year" style="font-size: 15px;height: 25px;padding-top: 0px;margin-top: 25px;width: 300px;">
										...
										<?php
											...
											$sql = "SELECT DISTINCT(month_year) FROM salary_month_details where bank_advice_status  = 'Pending'";
									...
									<label style="margin-left:-3%;color: #495057;margin-top:5%">Upload Bank Advice :  &nbsp; </label> 
														
									<input required ="true" accept = ".pdf,application/pdf" class="toggle btn btn-primary" type="File" id="bank_document" name = "bank_document" value= "" autofocus style="margin-bottom: 7px;"><!--span class="text-success">&nbsp;<?php echo $uidaiproof;?></span--></input></br>
	...
	<script>
		...
		function uploadme()
		{
			...
			if(allowedPdfExtensions.exec(bankdoc) == ".pdf,.pdf" && date !=="")
			{

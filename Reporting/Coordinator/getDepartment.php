...
{
  $query ="SELECT DISTINCT(TRIM(dept)) FROM tbl_attendance WHERE state = '" . $_POST["state"]."' order by dept asc";
...
{
  $query ="SELECT DISTINCT(TRIM(dept)) FROM tbl_attendance WHERE state = '" . $_POST["state"]."' and district = '".$district_name."' order by dept asc";

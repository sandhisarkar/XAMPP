...
$q1=mysqli_query($con,"select sum(gross) from salary_month_details where pf_esi_challan_status = 'Completed' AND bank_advice_status = 'Completed' ");
...
if(isset($_POST["excel_sub"]))
{
	...
	if($operator == "ALL")
	{
		$sql = "SELECT DISTINCT(MONTH_YEAR) FROM salary_month_details WHERE created_dttm >=  '".$stDt1."' AND created_dttm <= '".$ymd."' AND pf_esi_challan_status = 'Completed' AND bank_advice_status = 'Completed' ORDER BY MONTH_YEAR ASC";
		...
		$msg = "ok";
			...
			$q1=mysqli_query($con,"select sum(gross) from salary_month_details WHERE created_dttm >=  '".$stDt1."' AND created_dttm <= '".$ymd."' AND pf_esi_challan_status = 'Completed' AND bank_advice_status = 'Completed' ORDER BY MONTH_YEAR ASC");
			$n1=  mysqli_fetch_row($q1);
			$totamt1= $n1[0];
	...
	{
		$sql = "SELECT DISTINCT(MONTH_YEAR) FROM salary_month_details WHERE created_dttm >=  '".$stDt1."' AND created_dttm <= '".$ymd."' and operator_id = '".$operator_id."' AND pf_esi_challan_status = 'Completed' AND bank_advice_status = 'Completed' ORDER BY MONTH_YEAR ASC";
		  $result = mysqli_query($con, $sql);
		//echo $sql;
		if (mysqli_num_rows($result) > 0) {			
		...
		$msg = "ok";
			...
			$q1=mysqli_query($con,"select sum(gross) from salary_month_details WHERE created_dttm >=  '".$stDt1."' AND created_dttm <= '".$ymd."' AND operator_id = '".$operator_id."' AND pf_esi_challan_status = 'Completed' AND bank_advice_status = 'Completed' ORDER BY MONTH_YEAR ASC");
...
<html lang="en">
  ...
  <script>
      ...
      <div id="content" class="p-4 p-md-5 pt-5" >
        ...
        <h2 class="mb-4"><u>Operators Payment Summary</u> </h2>
				...
				<div class="row">
					  ...
					  <div class="col-sm-12  wow bounceInUp" data-wow-duration="1.4s">
						<div class="box">
						  ...
						  <h4 class="title" style="margin:0%;text-align: center;">Total Gross Amount Till Date <i class="fa fa-calendar text-info"></i></h4>
					  ...
					  <div class="col-sm-12  wow bounceInUp" data-wow-duration="1.6s" style="padding-top:5%;">
					  
							<form  action = "Operator Billing.php" method ="post"  id = "export_excel" enctype="multipart/form-data">
						    <div>
							...
							<div style="padding-top: 1%;">
								<i class="fa fa-user-o"></i>&nbsp;<label>Choose Operator : &nbsp; </label>
									<!--class="btn btn-primary dropdown-toggle" -->
									<select   required="true" id="operator" name="operator" onclick="operatorCheck()">
											<option selected value="" disabled>Select Any...</option>
											<option value="ALL">ALL</option>
											<option  value="Individual">Individual</option>
									</select><span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
								<i class="fa fa-keyboard-o"></i>&nbsp;<label>Operator ID : &nbsp; </label><input required ="true" type ="text" id="operator_id" name ="operator_id" autocomplete="off" placeholder= "Enter Operator ID" autofocus></input>
							</div>
					 ...
					 ?>
					 <?php
					 if($msg == "ok" && $operator == "ALL" && $operator_id == "")
					 {
					  ...
					  <div class="col-md-12  wow bounceInUp" data-wow-duration="2.4s" style="margin-top: 3%;">
						  <div class="box" style ="padding:0%; box-shadow: 0 0 white;">
							...
							<h4><i class="fa fa-rupee text-success"></i> Total Gross Amount :  <?php echo $totBillamt; ?> /-</h4>
					 ...
					 $result = mysqli_query($con, "SELECT DISTINCT(MONTH_YEAR) FROM salary_month_details WHERE created_dttm >=  '".$stDt1."' AND created_dttm <= '".$ymd."' AND pf_esi_challan_status = 'Completed' AND bank_advice_status = 'Completed' ORDER BY MONTH_YEAR ASC");
					 while($row = mysqli_fetch_row($result)) {
						 
						 $month_yr = $row[0];
						 
						  $result1 = mysqli_query($con, "SELECT sum(gross),Count(operator_id) FROM salary_month_details WHERE month_year =  '".$month_yr."' ");
					...
					<div class="col-md-6  wow bounceInUp" data-wow-duration="1.4s">
						<div class="box">
						  ...
						  <h4 class="title" style="margin:0%;text-align: center;">Gross Salary Month-Year : <u id="billMonth<?php echo $count;?>"><?php echo $month_yr; ?></u>  <span><i class="fa fa-calendar text-info"></i></span></h4>
						...
						<div class="col-mb-6 wow bounceInUp displayNone" id = "tableBill<?php echo $count;?>" data-wow-duration="1.6s" style="display:none;">
							...
							<thead class="thead-dark">
								<tr>
									...
									<th style = 'padding:auto;'>Number of Operators  <i class="fa fa-user-plus text-info"></i></th>
									<th style = 'padding:auto;'>Total Gross Amount  <i class="fa fa-rupee text-success"></i></th>	
								</tr>
							</thead>
							...
							$query = "SELECT DISTINCT(dist_name)  FROM salary_month_details WHERE month_year =  '".$month_yr."' ";
							 $result1 = mysqli_query($con, $query);
							 //echo $query;
							while($row = mysqli_fetch_row($result1)) {
								$dis_distname = $row[0];
								
							 $query1 = "SELECT COUNT(operator_id),SUM(gross) FROM salary_month_details WHERE month_year =  '".$month_yr."' AND dist_name = '".$dis_distname."' ";
					...
					<?php
					 if($msg == "ok" && $operator == "Individual" && $operator_id != "")
					 {
					  ...
					  <div class="col-md-12  wow bounceInUp" data-wow-duration="2.4s" style="margin-top: 3%;">
						  <div class="box" style ="padding:0%; box-shadow: 0 0 white;">
							<h6><i class="fa fa-search text-info"></i> <u>Showing Result For :  [<?php echo $_POST['start_date'];?>] - [<?php echo $_POST['end_date'];?>]</u></h6>
							<h4><i class="fa fa-rupee text-success"></i> Total Gross Amount For Operator (<?php echo $operator_id;?>):  <?php echo $totBillamt; ?> /-</h4>
						  </div>
					  </div>
					 <?php
					 $count=0;
					 
					 $result = mysqli_query($con, "SELECT DISTINCT(MONTH_YEAR) FROM salary_month_details WHERE created_dttm >=  '".$stDt1."' AND created_dttm <= '".$ymd."' AND operator_id = '".$operator_id."' AND pf_esi_challan_status = 'Completed' AND bank_advice_status = 'Completed' ORDER BY MONTH_YEAR ASC");
					 while($row = mysqli_fetch_row($result)) {
						 
						 $month_yr = $row[0];
						 
						  $result1 = mysqli_query($con, "SELECT sum(gross),sum(basic),sum(hra),sum(conv),sum(allow),sum(holamt),sum(amt_to_bank),SUM(working_days),sum(paytoop),sum(net),sum(empee_pf),sum(empee_esi),sum(empr_pf),sum(empr_esi),sum(adv_adj),sum(agg_adj) FROM salary_month_details WHERE month_year =  '".$month_yr."' AND operator_id = '".$operator_id."'");
						 while($row1 = mysqli_fetch_row($result1)){
						 $bill_amt = number_format($row1[0],2);
						 //echo $bill_amt;
						 $centre_count = $row1[1];
					...
					<div class="col-md-6  wow bounceInUp" data-wow-duration="1.4s">
						<div class="box">
						  ...
						  <h4 class="title" style="margin:0%;text-align: center;">Gross Salary Month-Year : <u id="billMonth<?php echo $count;?>"><?php echo $month_yr; ?></u>  <span><i class="fa fa-calendar text-info"></i></span></h4>
						  <h1 class="description" style="text-align: center;margin:0%;margin-top: revert;font-size: 17px;"><i class="fa fa-rupee text-success" style= "font-size: large;"></i><span>&nbsp;</span> <?php echo $bill_amt; ?> /-</h1>
						  <h2 class="description" style="text-align: center;margin:0%;margin-top: revert;font-size: 17px;line-height: 0px;"><i class="fas fa-running text-warning"></i> <span>Number of Working Days :  <?php  echo $work; ?> <span>&nbsp;</span><i class="fa fa-calendar text-info"></i></h2>
						  <h2 class="description" style="text-align: center;margin:0%;margin-top: revert;font-size: 17px;line-height: 0px;">Basic : <i class="fa fa-rupee text-success"></i><span>&nbsp;</span> <?php echo number_format($row1[1],2); ?> /-</h2>
						  <h3 class="description" style="text-align: center;margin:0%;margin-top: revert;font-size: 17px;line-height: 0px;">HRA : <i class="fa fa-rupee text-success"></i><span>&nbsp;</span> <?php echo number_format($row1[2],2); ?> /-</h3>
						  <h3 class="description" style="text-align: center;margin:0%;margin-top: revert;font-size: 17px;line-height: 0px;">Conveyance : <i class="fa fa-rupee text-success"></i><span>&nbsp;</span> <?php echo number_format($row1[3],2); ?> /-</h3>
						  <h3 class="description" style="text-align: center;margin:0%;margin-top: revert;font-size: 17px;line-height: 0px;">Allowance : <i class="fa fa-rupee text-success"></i><span>&nbsp;</span> <?php echo number_format($row1[4],2); ?> /-</h3>	
						  <h3 class="description" style="text-align: center;margin:0%;margin-top: revert;font-size: 17px;line-height: 0px;">Hold Amount : <i class="fa fa-rupee text-success"></i><span>&nbsp;</span> <?php echo number_format($row1[5],2); ?> /-</h3>
						  <h3 class="description" style="text-align: center;margin:0%;margin-top: revert;font-size: 17px;line-height: 0px;">Payable Amount : <i class="fa fa-rupee text-success"></i><span>&nbsp;</span> <?php echo number_format($row1[8],2); ?> /-</h3>
						  <h3 class="description" style="text-align: center;margin:0%;margin-top: revert;font-size: 17px;line-height: 0px;">Net Amount : <i class="fa fa-rupee text-success"></i><span>&nbsp;</span> <?php echo number_format($row1[9],2); ?> /-</h3>
						  <h3 class="description" style="text-align: center;margin:0%;margin-top: revert;font-size: 17px;line-height: 0px;">Employee's PF Contribution : <i class="fa fa-rupee text-success"></i><span>&nbsp;</span> <?php echo number_format($row1[10],2); ?> /-</h3>
						  <h3 class="description" style="text-align: center;margin:0%;margin-top: revert;font-size: 17px;line-height: 0px;">Employee's ESI Contribution : <i class="fa fa-rupee text-success"></i><span>&nbsp;</span> <?php echo number_format($row1[11],2); ?> /-</h3>
						  <h3 class="description" style="text-align: center;margin:0%;margin-top: revert;font-size: 17px;line-height: 0px;">Employer's PF Contribution : <i class="fa fa-rupee text-success"></i><span>&nbsp;</span> <?php echo number_format($row1[12],2); ?> /-</h3>
						  <h3 class="description" style="text-align: center;margin:0%;margin-top: revert;font-size: 17px;line-height: 0px;">Employer's ESI Contribution : <i class="fa fa-rupee text-success"></i><span>&nbsp;</span> <?php echo number_format($row1[13],2); ?> /-</h3>
						  <h3 class="description" style="text-align: center;margin:0%;margin-top: revert;font-size: 17px;line-height: 0px;">Advance Adjustment Amount : <i class="fa fa-rupee text-success"></i><span>&nbsp;</span> <?php echo number_format($row1[14],2); ?> /-</h3>
						  <h3 class="description" style="text-align: center;margin:0%;margin-top: revert;font-size: 17px;line-height: 0px;">Aggregate Adjustment Amount : <i class="fa fa-rupee text-success"></i><span>&nbsp;</span> <?php echo number_format($row1[15],2); ?> /-</h3>
						  <h3 class="description" style="text-align: center;margin:0%;margin-top: revert;font-size: 17px;line-height: 0px;">Amount Deposited to Bank : <i class="fa fa-rupee text-success"></i><span>&nbsp;</span> <?php echo number_format($row1[6],2); ?> /-</h3>	
						</div>
						...
						<div class="col-mb-6 wow bounceInUp displayNone" id = "tableBill<?php echo $count;?>" data-wow-duration="1.6s" style="display:none;">
							...
							//$abc = '<script>document.write(document.getElementById("#txtSequenceId"));</script>';
							//echo $_SESSION["txtSequenceId"][$count];
							$query = "SELECT DISTINCT(dist_name) FROM salary_month_details WHERE month_year =  '".$month_yr."' and ven_name = '".$vendor_name."'";
							 $result1 = mysqli_query($con, $query);
							 //echo $query;
							while($row = mysqli_fetch_row($result1)) {
								$dis_distname = $row[0];
								
							 $query1 = "SELECT COUNT(centre_name),SUM(ven_bill_per_month_per_op) FROM salary_month_details WHERE month_year =  '".$month_yr."' AND dist_name = '".$dis_distname."' and ven_name = '".$vendor_name."'";
							 $result2 = mysqli_query($con, $query1);
							 $row1 = mysqli_fetch_row($result2);
								?>
								<tbody>
	...
	<script>
		...
		function uploadme()
		{
			...
			var opstat = $("#operator").val();
			var op_id = $("#operator_id").val();
			
			if(start_month_year !== "" && end_month_year !== "" && ((opstat === "ALL" && op_id ==="") ||(opstat === "Individual" && op_id != "")))
			{
			...
			if(start_month_year === null || end_month_year === null)
			{
				document.getElementById("overlay").style.display = "none";
				//$("#search").hide();
				return false;

<?php
session_start();
//header( "refresh:30;url=/Bihar_PEC" );
...
if(isset($_REQUEST["u_sub"]))
{
 ...
 if($usr!=''&&$pwd!='')
 {
   $query=mysqli_query($con ,"select * from admin_user where admin_user='".$usr."' and admin_pwd='".$pwd."'");
   $res=mysqli_fetch_row($query);
   //$query1=mysqli_query($con ,"select * from  accounts_user where accounts_user='".$usr."' and accounts_pwd='".$pwd."''");
   //$res1=mysqli_fetch_row($query1);
   /*$query2=mysqli_query($con ,"select * from  vendor_details where s_user='".$usr."'");
   $res2=mysqli_fetch_row($query2);*/
   if($res)
   {
    $_SESSION['user']=$usr;
	
    header('location:/Bihar_PEC/Admin/');
   }
   else
   {
...
<html>
    <head>
        ...
        <title>Bihar PEC Portal</title>
        
		<!-- Favicons -->
		<link href="xyz.png" rel="icon">
		<link href="xyz.png" rel="apple-touch-icon">
    ...
    <body  style="background-image:url('./images/06.jpg'); background-size: cover; background-repeat: no-repeat;" >

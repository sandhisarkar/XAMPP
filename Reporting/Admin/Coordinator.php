...
if(isset($_POST["sub_pass"]))
{
    ...
    if($state != "" && $district != "" && $username != "")
    {
       $sql_sl = "SELECT COUNT(slno) FROM tbl_coordinator";
       $query_sl = mysqli_query($con, $sql_sl);
       $result_sl = mysqli_fetch_row($query_sl);
       $count = $result_sl[0];
       ...
       {
        $emp_role = 1;
        $coid = strtoupper(substr($state,0,2)."_".str_pad($slno,3,"0",STR_PAD_LEFT));
       }   
       else
       {
        $emp_role = 2;
        $coid = strtoupper(substr($state,0,2)."_".substr($district,0,3)."_".str_pad($slno,3,"0",STR_PAD_LEFT));
       }    

        $insert_sql="INSERT INTO tbl_coordinator (slno,`state`,district,username,co_id,`password`,role_id,activity_status) VALUES ('".$slno."','".$state."','".$district."','".$username."','".$coid."','password','".$emp_role."','1')";
  ...
  <body>
          ...
          <ul id="side-main-menu" class="side-menu list-unstyled">                  
            ...
            <li class="active"><a href="#exampledropdownDropdownCo" aria-expanded="false" data-toggle="collapse"> <i class="fa fa-user"></i>Coordinator </a>
              <ul id="exampledropdownDropdownCo" class="collapse list-unstyled ">
                <li class="active"><a href="Coordinator.php" onclick="openOverlay()"> <i class="fa fa-user-plus"></i>Create Coordinator</a></li>
    ...
    <div class="page">
      ...
      <section class="forms">
        <div class="container-fluid">
          <!-- Page Header-->
          <header> 
            <h1 class="h3 display">Create Coordinator . . .</h1>
            <p>(To Create New Coordinator)</p>
          </header>
          <div class="row">
            <div class="col-lg-4">
            </div>
                ...
                <div class="card-body">
                  ...
                  <form style="border-style: none;" action = "" method ="post"  id = "password" enctype="multipart/form-data">
                            ...
                            <label>Choose State </label>
                                    ...
                                    $sql = "SELECT Distinct(state) FROM tbl_operator where state != ''  order by state ASC";
                            ...
                            <label>Username </label>
                                ...
                                placeholder="Enter Coordinator Name" id="username" name = "username" class="form-control">
                            </div> 
                            <div class="form-group">       
                                <input type="submit" value="Create Coordinator" style="float: right;"  class="btn btn-primary" id="sub_pass" name = "sub_pass" onclick = "uploadme()">
                            </div>
            ...
            <div class="col-sm-12" style = "text-align: center; margin-left: auto;margin-right: auto;">
					<?php
                        ...
                        if($error == "Query")
						{
							//header( "refresh:3;url=Operator.php" );
							echo '<div class="spinner-grow spinner-grow-sm"> </div><span> &nbsp;&nbsp;&nbsp;</span>';
							echo '<label class="text-danger">Error creating new Coordinator </label>';
                        ...
                        if($success == "Success")
						{
							//header( "refresh:3;url=Operator.php" );
							echo '<div class="spinner-grow spinner-grow-sm"> </div><span> &nbsp;&nbsp;&nbsp;</span>';
							echo '<label class="text-success">New Coordinator created successfully ... </label>';
							//header( "refresh:3;url=Operator.php" );
						}
					?>
				</div>
    ...
    <script language ="javascript">
		function uploadme()
		{
          ...
          if(state !== null && district !== null && username !== null)
          {

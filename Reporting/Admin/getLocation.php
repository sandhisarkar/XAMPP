...
{
  $query ="SELECT DISTINCT(TRIM(pec_location)) FROM tbl_attendance WHERE state = '" . $_POST["state"]."' order by pec_location asc";
...
{
  $query ="SELECT DISTINCT(TRIM(pec_location)) FROM tbl_attendance WHERE state = '" . $_POST["state"]."' and district = '".$district_name."' order by pec_location asc";

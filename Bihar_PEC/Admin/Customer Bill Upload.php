...
if(isset($_POST["sub_excel"]))
{
	...
	if(!empty($_FILES["cus_bill_document"]))
	{
		if(($_FILES["cus_bill_document"]["type"] == ".pdf" || $_FILES["cus_bill_document"]["type"] == "application/pdf"))
		{
			chdir("..");
			
			
			$cusbilldocpath = "Admin/Documents/Customer Billing/".$month_yr."/";
			
			
			if(!is_dir($cusbilldocpath))
			{
				//echo 1;
				mkdir($cusbilldocpath,"0777",true);
			...
			$cusbilldocpath=$cusbilldocpath.$_FILES['cus_bill_document']['name'];
			
			
			if(move_uploaded_file($_FILES['cus_bill_document']['tmp_name'],$cusbilldocpath))
			{
				$cusbilldoc = $_FILES['cus_bill_document']['name'];
				
				
				$sql="UPDATE salary_month_details SET customer_bill_status = 'Completed', customer_bill_doc = '".$cusbilldocpath."' WHERE month_year='".$month_yr."'";
...
<html lang="en">
  ...
  <script>
      ...
      <div id="content" class="p-4 p-md-5 pt-5" >
        ...
        <h2 class="mb-4"><u>Upload Customer Invoice</u> (Monthly)</h2>
			...
			<div class="col-sm-12" style = "margin-top:5%;">
				...
				<div class="col-sm-6" style = "text-align: center; margin-left: auto;margin-right: auto;">
					...
					<center>
						...
						<form  action = "Customer Bill Upload.php" method ="post"  id = "export_excel" enctype="multipart/form-data">
							<table>
								...
								<td style = "text-align: center;margin-left: auto;margin-right: auto;">
									...
									<select required="true" id="month_year" name="month_year" style="font-size: 15px;height: 25px;padding-top: 0px;margin-top: 25px;width: 300px;">
										...
										<?php
											...
											$sql = "SELECT DISTINCT(month_year) FROM salary_month_details where customer_bill_status  = 'Pending'";
									...
									<label style="margin-left:-18%;color: #495057;margin-top:5%">Upload Customer Billing Advice :  &nbsp; </label> 
														
									<input required ="true" accept = ".pdf,application/pdf" class="toggle btn btn-primary" type="File" id="cus_bill_document" name = "cus_bill_document" value= "" autofocus style="margin-bottom: 7px;"><!--span class="text-success">&nbsp;<?php echo $uidaiproof;?></span--></input></br>
									
									
									<button class="toggle btn btn-primary" onclick = "uploadme()" type="submit" id="sub_excel" name = "sub_excel"
									style="font-family: Verdana;  margin-top:15px; background-color: #1163aa; border: transparent; margin-bottom: 15px;">Upload Document<i class="fa fa-upload"></i></button>
	...
	<script>
		...
		function uploadme()
		{
			var cusbilldoc = $("#cus_bill_document").val();
			...
			if(allowedPdfExtensions.exec(cusbilldoc) == ".pdf,.pdf" && date !=="")
			{

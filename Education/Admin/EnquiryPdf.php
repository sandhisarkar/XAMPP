...
function viewTable($con)
{
	...
	$sql = "SELECT * from enquiry_details where centre_name = '".$_GET["centre_name"]."' and centre_location = '".$_GET["centre_location"]."' and created_dttm >= '".$_GET["start_date"]."' and created_dttm <= '".$_GET["start_end"]."'";
...
$pdf->SetFont('Times','',12);
$pdf->Cell(1);
$pdf->Cell(276,10,'Centre Name : '.$_GET["centre_name"],2,0,'C');
$pdf->Ln();
$pdf->SetFont('Times','',12);
$pdf->Cell(1);
$pdf->Cell(276,10,'Centre Location : '.$_GET["centre_location"],2,0,'C');
$pdf->Ln(20);
...
$pdf->Output('Enquiry_'.$_GET["centre_location"].'-'.$_GET["centre_name"].'.pdf','D');
}
if($type == 'csv')
{
	$sql = "SELECT * from enquiry_details where centre_name = '".$_GET["centre_name"]."' and centre_location = '".$_GET["centre_location"]."' and created_dttm >= '".$_GET["start_date"]."' and created_dttm <= '".$_GET["start_end"]."'";
	...
	$filename = "Enquiry_".$_GET["centre_location"].'-'.$_GET["centre_name"].".csv";

...
$sql="UPDATE operator_profile SET uidai_no='".$_POST['uidai_no']."',pan = '".$_POST['pan_no']."',aadhar_no='".$_POST['aadhar_no']."' WHERE sl_no='".$op_sl."' AND operator_id = '".$opid."'";
  ...
  <body>
			...
			<div class="col-sm-12" style = "margin-top:0%;">
				...
				<div class="col-sm-12" style = "text-align: center; margin-left: auto;margin-right: auto;">
					...
					<?php
						...
						if($msgNav == "Error")
						{
							header("refresh:1;url=Operator ID.php" );	
					...
					<ul class="pagination pagination-lg" style="display: inline flow-root list-item; padding-bottom: 1%;">
						...
						?>
						<li class="active"><a onclick= "openOverlay()" href="Operator ID.php?Handle=<?php echo  $op_sl ;?>" data-toggle="tooltip" title="Operator ID Information" data-placement="bottom"><i class="fa fa-id-card-o" aria-hidden="true"></i></a></li>						
						<?php
				...
				<div class="col-sm-12" style = "text-align: center; margin-left: auto;margin-right: auto;">
					<center>
						<form  action = "Operator ID.php?Handle=<?php echo  $op_sl;?>" method ="post"  id = "export_excel" enctype="multipart/form-data">
						<div class="col-sm-5">
							<div class="input-group mb-3">
								<div class="input-group-append">
									  <span class="input-group-text" style="background-color:transparent;border:0;">Certification Number :</span>
								</div>
								
								<input value ="<?php echo $uidai;?>" class = "form-control" pattern = "^[A-Z0-9]+$" type="text" id="uidai_no" name="uidai_no" required="true" placeholder ="Enter Certification Number" autocomplete="off" style ="border-radius:5px;" autofocus></input>
							 </div>
						</div>
						<div class="col-sm-5">
							<div class="input-group mb-3">
								<div class="input-group-append">
									  <span class="input-group-text" style="background-color:transparent;border:0;margin-left: 55px;">PAN Number :</span>
								</div>
								
								<input value ="<?php echo $pan;?>" class = "form-control" pattern = "^([A-Z]){5}([0-9]){4}([A-Z]){1}?$" type="text" id="pan_no" name="pan_no" required="true" placeholder ="Enter PAN Number"autocomplete="off" style ="border-radius:5px;" autofocus></input></br></br>
							 </div>
						</div>
						<div class="col-sm-5">
							<div class="input-group mb-3">
								<div class="input-group-append">
									  <span class="input-group-text" style="background-color:transparent;border:0;margin-left: 35px;">Aadhar Number :</span>
								</div>
								
								<input value ="<?php echo $aadhar;?>" class = "form-control" pattern = "^\d{4}\s\d{4}\s\d{4}$" type="text" id="aadhar_no" name="aadhar_no" autocomplete="off" placeholder ="Enter Aadhar Number" required="true" style ="border-radius:5px;" autofocus></input>
							 </div>
						</div>
						<input class="toggle btn btn-primary" onclick="uploadme()" type="submit" id="update_id" name = "update_id" value="Update Operator Id" style="font-family: Verdana;  margin-top:15px; background-color: #1163aa; border: transparent; margin-bottom: 15px;">
	...
	<script>
		...
		function uploadme()
		{
			...
			var uidai_no = $("#uidai_no").val();
			var pan_no = $("#pan_no").val();
			var aadhar_no = $("#aadhar_no").val();
			
			
			var patt = new RegExp("^[A-Z0-9]+$");
			var res = patt.test(uidai_no);
			
			var patt1 = new RegExp("^([A-Z]){5}([0-9]){4}([A-Z]){1}?$");
			var res1 = patt1.test(pan_no);
			
			var patt2 = new RegExp("^[0-9]{4}[ \t\n\r][0-9]{4}[ \t\n\r][0-9]{4}$");
			var res2 = patt2.test(aadhar_no);
			
			
			
			if(uidai_no !== "" && pan_no !== ""  && aadhar_no !== "" &&  res == true && res1 == true && res2 == true)
			{
				document.getElementById("overlay").style.display = "block";

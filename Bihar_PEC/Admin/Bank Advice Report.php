...
if(isset($_POST["excel_sub"]))
{
	...
	$monyr= $_POST["month_year"];
		...
		$phpExcel ->getProperties()->setTitle("Monthly Operators Salary Payment Bank Advice List");
		$phpExcel ->getProperties()->setCreator("Sandhi Sarkar");
		$phpExcel ->getProperties()->setDescription("Excel for Operators Salary Payment Bank Advice");
		...
		$sheet ->getCell('F1')->setValue('Monthly Operators Salary Payment Bank Advice');
		$sheet->getStyle('F1')->getFont()->setBold(true)->setSize(12);
		
		$sheet ->getCell('A3')->setValue('Month-Year');
		$sheet ->getCell('B3')->setValue('District Name');
		$sheet ->getCell('C3')->setValue('Value Date');
		$sheet ->getCell('D3')->setValue('Serial No.');
		$sheet ->getCell('E3')->setValue('Amount(Rs)');
		$sheet ->getCell('F3')->setValue('Name of the Account Holder');
		$sheet ->getCell('G3')->setValue('Bank Name');
		$sheet ->getCell('H3')->setValue('Branch Name');
		$sheet ->getCell('I3')->setValue('Account Number');
		$sheet ->getCell('J3')->setValue('IFSC Code');
		$sheet ->getCell('K3')->setValue('UTR No');
		$sheet ->getCell('L3')->setValue('Bank Advice Status');
		...
		$result1 = mysqli_query($con, "SELECT DISTINCT(MONTH_YEAR) FROM salary_month_details WHERE month_year = '".$monyr."' ORDER BY MONTH_YEAR ASC");
		 ...
		 $result2 = mysqli_query($con, "SELECT amt_to_bank,operator_name,bank_name,branch_name,acc_no,ifsc,bank_advice_status FROM salary_month_details WHERE month_year =  '".$month_yr."' AND dist_name = '".$distname."'");
		...
		{
			$status = "Approved but Payment Pending";
		}
		else
		{
			$status = "Payment Made";
		...
		// Insert product data
		$sheet ->getCell('A'.$count.'')->setValue($month_yr);
		$sheet ->getCell('B'.$count.'')->setValue($distname);
		$sheet ->getCell('C'.$count.'')->setValue();
		$sheet ->getCell('D'.$count.'')->setValue($sl_no);
		$sheet ->getCell('E'.$count.'')->setValue($amttobank);
		$sheet ->getCell('F'.$count.'')->setValue($opname);
		$sheet ->getCell('G'.$count.'')->setValue($bankname);
		$sheet ->getCell('H'.$count.'')->setValue($branchname);
		$sheet ->getCell('I'.$count.'')->setValue($accno);
		$sheet ->getCell('J'.$count.'')->setValue($ifsc);
		$sheet ->getCell('K'.$count.'')->setValue();
		$sheet ->getCell('L'.$count.'')->setValue($status);
		...
		$filename = "Monthly_Operators_Salary_Payment_Bank_Advice_".date('Ymd');
...
<html lang="en">
  ...
  <script>
      ...
      <div id="content" class="p-4 p-md-5 pt-5" >
        ...
        <h2 class="mb-4"><u>Monthly Operators Salary Payment Bank Advice </u> </h2>
								...
								<label style="margin-left: -30px;color: #495057;"><i class="fa fa-calendar"></i>&nbsp; Choose Month-Year : &nbsp; </label>
									<select required="true" id="month_year" name="month_year" style="font-size: 15px;height: 25px;padding-top: 0px;margin-top: 25px;width: 115px;">
										...
										<?php
											...
											$sql = "SELECT DISTINCT(month_year) FROM salary_month_details where bank_advice_status  = 'Pending' OR bank_advice_status  = 'Created' order by month_year ASC";
											$result = mysqli_query($con, $sql);
											if (mysqli_num_rows($result) > 0) {
							...
							<div style="padding-top: 1%;">
								<i class="fa fa-map"></i>&nbsp;<label>Choose Location : &nbsp; </label>
									...
									<select   required="true" id="dist_name" name="dist_name" autofocus>
											...
											$sql = "SELECT Distinct(dist_name) FROM salary_month_details where bank_advice_status  = 'Pending' OR bank_advice_status  = 'Created' order by dist_name ASC";
											$result = mysqli_query($con, $sql);
											if (mysqli_num_rows($result) > 0) {
											// output data of each row
					...
					<?php
					 if($msg == "not ok")
					 {
					  ...
					  <div class="col-md-12  wow bounceInUp" data-wow-duration="2.4s" style="margin-top:5%;">
						  <div class="box">
							<h4 class="title"><i class="fa fa-search text-info"></i> No Salary Payment(Bank Advice) Record Found. . . </h4>

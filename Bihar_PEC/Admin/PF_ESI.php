...
if(isset($_POST["excel_sub"]))
{
	...
	$monyr= $_POST["month_year"];
		...
		$phpExcel ->getProperties()->setTitle("Monthly PF/ESI Payment Advice List");
		$phpExcel ->getProperties()->setCreator("Sandhi Sarkar");
		$phpExcel ->getProperties()->setDescription("Excel for Monthly PF/ESI Payment Advice");
		...
		$sheet->setTitle('Monthly PF_ESI Payment Report');
		
		// Creating spreadsheet header
		$sheet ->getCell('I1')->setValue('Monthly PF/ESI Payment Advice');
		...
		$sheet ->getCell('Q3')->setValue('Employer Contribution to PF');
		$sheet ->getCell('R3')->setValue('Employer Contribution to ESI');
		$sheet ->getCell('S3')->setValue('PF/ESI Challan Status');
		
		// Making headers text bold and larger
		
		$sheet->getStyle('A3:S3')->getFont()->setBold(true)->setSize(11);
		...
		$result1 = mysqli_query($con, "SELECT DISTINCT(MONTH_YEAR) FROM salary_month_details WHERE created_dttm >=  '".$stDt1."' AND created_dttm <= '".$ymd."' ORDER BY MONTH_YEAR ASC");
		 ...
		 $result2 = mysqli_query($con, "SELECT state_name,dist_name,centre_name,operator_name,operator_id,station_id,working_days,basic,hra,conv,allow,gross,empee_pf,empee_esi,net,empr_pf,empr_esi,pf_esi_challan_status FROM salary_month_details WHERE month_year =  '".$month_yr."' ");
		...
		$sheet ->getCell('Q'.$count.'')->setValue($empr_pf);
		$sheet ->getCell('R'.$count.'')->setValue($empr_esi);
		$sheet ->getCell('S'.$count.'')->setValue($status);
		...
		$filename = "Monthly_PF_ESI_".date('Ymd');
...
<html lang="en">
  ...
  <script>
      ...
      <div id="content" class="p-4 p-md-5 pt-5" >
        ...
        <h2 class="mb-4"><u>Monthly PF/ESI Payment Report </u> </h2>
				...
				<div class="row">
					...
					<?php
					 if($msg == "not ok")
					 {
					  ...
					  <div class="col-md-12  wow bounceInUp" data-wow-duration="2.4s" style="margin-top:5%;">
						  <div class="box" >
							<h6 class="title" ></i> Showing Result For :  <u>[<?php echo $_POST['start_date'];?>] </u> - <u> [<?php echo $_POST['end_date'];?>]</u><i class="fa fa-calendar text-info"></i></h6>
							<h6 class="description" ><i class="fa fa-search text-info"></i> No PF/ESI Payment Advice Found </h6>
						  </div>
					  </div>
					 <?php
					 }
					...
					<?php
					 if($msg == "ok")
					 {
					  ...
					  <div class="col-md-12  wow bounceInUp" data-wow-duration="2.4s" style="margin-top:5%;">
						  <div class="box">
							<h6 class="title"></i> Showing Result For :  <u>[<?php echo $_POST['start_date'];?>] </u> - <u> [<?php echo $_POST['end_date'];?>]</u> <i class="fa fa-calendar text-success"></i></h6>
							<h6 class="description" ><i class="fa fa-search text-success"></i>PF/ESI Payment Advice Generated Successfully </h6>
						  </div>
					  </div>
					 <?php
					 }
